...
def doInstall():
    ...
    try:
        ...
        for file in [ 'install.sh', 'install.py', 'install.log', 'update.sh', 'Package.list', 'config/heating.conf']:
            os.remove("%s%s" % (newlocation, file))
        for folder in ['bin/at', 'service']:
            shutil.rmtree("%s%s" % (newlocation, folder))
